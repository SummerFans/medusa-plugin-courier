...
export default async (
  ...
  notificationService.subscribe(OrderService.Events.RETURN_ACTION_REQUIRED, providerId);  // Triggered when the order is being registered as “returned” but there are additional actions required related to refunding the payment.
  notificationService.subscribe(OrderService.Events.REFUND_CREATED, providerId);          // Triggered when the order’s payment is refunded.
  notificationService.subscribe(OrderService.Events.REFUND_FAILED, providerId);           // Triggered when a refund fails.
  notificationService.subscribe(OrderService.Events.PAYMENT_CAPTURED, providerId);        // Triggered when the payment of an order is captured.
  notificationService.subscribe(OrderService.Events.PAYMENT_CAPTURE_FAILED, providerId);  // Triggered when capturing the payment of an order fails.
  notificationService.subscribe(OrderService.Events.FULFILLMENT_CREATED, providerId);     // Triggered when a shipment is created for fulfillment and the fulfillment is registered as “shipped”.
  notificationService.subscribe(OrderService.Events.FULFILLMENT_CANCELED, providerId);    // Triggered when fulfillment of an order is canceled.
  notificationService.subscribe(OrderService.Events.SWAP_CREATED, providerId);            // Triggered when a swap for an order is created.
  notificationService.subscribe(OrderService.Events.UPDATED, providerId);                 // Triggered when an order and data associated with it (shipping method, shipping address, etc…) are updated.
  notificationService.subscribe(OrderService.Events.COMPLETED, providerId);                // Triggered when an order is completed.
  ...
  /* [Inventory Item Events] This section holds all events related to inventory items. */
